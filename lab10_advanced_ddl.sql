...
--alice - 100, wally + 100
--no we did rollback
--it will not be possible to roll back only part of the operation
...
--a) First SELECT shows original data, second SELECT shows only Fanta (old data removed)
--b) Both SELECTs show the same initial data, Terminal 2 changes are not visible
--c) READ COMMITTED shows other transactions' commits, SERIALIZABLE provides full isolation
